    ...
    public static function getFieldsMap() {
        if (!self::$fieldsMap) {
            self::$fieldsMap = [
                ...
                ['name'=>'ref_id','label'=>'资源ID','type'=>'text','default'=>isset($_GET['ref_id'])?$_GET['ref_id']:'','required'=>true,'field'=>function($model) {
                    if ($model->getData('ref_id')) {
                        return '<a target="_blank" href="/admin/'.$model->getData('ref').'?__filter='.urlencode('id='.$model->getData('ref_id')).'">'.$model->getData('ref_id').'</a>';
                    }
                }],
